    ...
    render() {
        ...
        const pendingInvitesList = pendingInvites.length ? (
            pendingInvites.map((invite) => {
                ...
                const actions = checkFriendship(invite, currentUser)
                    .actionsAvailable;
        ...
        function profileImage(user) {
          const checkedUser = users.find((userFromList) => userFromList._id === user._id);
        ...
        const friendsList = friends.length ? (
            friends.map((friend) => {
                const getAge = (birthDate) =>
                    Math.floor(
                        (new Date() - new Date(friend.birthDate).getTime()) /
                            3.15576e10
                    );
                return (
